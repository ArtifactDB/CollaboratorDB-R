...
Some objects may be stored multiple times under different names if they have an alias.

```{r}
identical(objects[["my_first_df"]], objects[["my_first_df/simple.csv.gz"]])
```

```{r, echo=FALSE}
stopifnot(identical(objects[["my_first_df"]], objects[["my_first_df/simple.csv.gz"]]))
```
...
However, these functions are relatively inefficient, so it is better to use `fetchObject()` explicitly in your scripts once the resource of interest is identified.
